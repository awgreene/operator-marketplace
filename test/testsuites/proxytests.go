...
// ProxyTests is a test suite that ensures that proxy functionality is working as intended.
func ProxyTests(t *testing.T) {
	t.Run("opsrc-registry", testOpSrcRegistryIncludesProxyVars)
	t.Run("csc-registry", testCscRegistryIncludesProxyVars)
}

// testOpSrcRegistry ensures that the Operator Registry deployment
// created by an OperatorSource has the appropriate environment
// variables set.
...
// testCSCRegistry ensures that the Operator Registry deployment
// created by an OperatorSource has the appropriate environment
// variables set.
...
func checkDeploymentIncludesProxyVars(t *testing.T, name string) error {
	...
	// Create an array of EnvVars using the cluster proxy.
	...
	// Sort and compare.
	proxy.SortEnvVars(expected)
	proxy.SortEnvVars(actual)
	if !proxy.EqualEnvVars(expected, actual) {
		return fmt.Errorf("Expected environment variables %v != actual %v", expected, actual)
	}
	return nil
}
